...
import sys
import json
sys.path.append(os.path.abspath('Gemini_mission-quizzify')) 
...
class QuizManager:
    ...
    def get_question_at_index(self, index: int):
        ...
        valid_index = index % self.total_questions # Ensure index is always within bounds using modulo arithmetic
    ...
    def next_question_index(self, direction=1):
        ...
        question_index = st.session_state['question_index']     # Retrieve the  question index from Streamlit's session state

        question_index = (question_index + direction) % self.total_questions 
    ...
    if question_bank:
        ...
        with st.container():
            ...
            with st.form("Multiple Choice Question"):
                ...
                st.write(f"You selected: ",index_question['question'])
                ...
                if submitted: # On click submit 
                    ...
                    for choice in index_question['choices']:
                       if answer == choice['value']: # Check if answer is correct
                        st.success("Correct!")
                    else:
                        st.error("Incorrect!")
            ##########################################################